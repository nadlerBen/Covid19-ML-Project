...
def main():
    ...
    #optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4, betas=(0.9, 0.99))
    ...
    for epoch in range(
        ...
        if total_test_acc > best_test_acc and total_train_acc > total_test_acc:
            ...
            torch.save(model.state_dict(), f'final_lstm_no_crf_pickle_one_month_world_model_{total_train_acc}_train_acc_{best_test_acc}_test_acc_{epoch}_epoch_allfeatures.pkl')
            torch.save(model.state_dict(), f'final_lstm_no_crf_one_month_world_model_{total_train_acc}_train_acc_{best_test_acc}_test_acc_{epoch}_epoch_allfeatures.pth')
            torch.save(model, f'final_lstm_no_crf_entire_model_one_month_world_{total_train_acc}_train_acc_{best_test_acc}_test_acc_{epoch}_epoch_allfeatures.pth')
